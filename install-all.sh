...
export HADOOP_CONF_DIR=$HADOOP_HOME/etc/hadoop' | sudo tee --append /home/ubuntu/.bashrc > /dev/null
...
export PYTHONPATH=$SPARK_HOME/python:$SPARK_HOME/python/lib/py4j-0.10.9-src.zip:$PYTHONPATH' | sudo tee --append /home/ubuntu/.bashrc > /dev/null
...
echo -e '$HADOOP_HOME/sbin/start-dfs.sh && $HADOOP_HOME/sbin/start-yarn.sh && $HADOOP_HOME/sbin/mr-jobhistory-daemon.sh start historyserver' > ~/hadoop-start-master.sh
echo '$SPARK_HOME/sbin/start-master.sh && $SPARK_HOME/sbin/start-slaves.sh' > ~/spark-start-master.sh
